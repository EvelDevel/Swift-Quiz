...
class ControlFlowSet {
    static func getQuestions() -> [Question] {
        return [
            ...
            Question(question: ["Переменная example не задействована, на что ее можно заменить?"],
                     image: "ControlFlow6",
            ...
            Question(question: ["Какие значения мы получим в консоль?",
                                "Какая очередность цифр выведется в консоль после запуска?"],
                     image: "ControlFlow8",
                     optionA: "2, 4, 6",
                     optionB: "0, 2, 4, 6",
                     optionC: "2, 4, 6, 8",
            ...
            Question(question: ["Первая итерация цикла while будут выполняться только в этом случае",
                                "Первая итерация цикла while будет исполняться, если ...?",
            ...
            Question(question: ["Первая итерация цикла repeat-while будут выполняться только в этом случае",
                                "Первая итерация цикла repeat-while будет исполняться, если ...?",
            ...
            Question(question: ["В каком случае инструкция if выполнит установленные инструкции?",
                                ...
                                "В каком случае будет выполнятся тело инструкции if?"],
                     image: "",
            ...
            Question(question: ["Что выведется в консоль?",
                                "Какая цифра напечатается в консоль?"],
                     image: "ControlFlow23",
                     optionA: "2",
                     optionB: "1",
                     optionC: "3",
            ...
            Question(question: ["Если при проверке по switch-case ваше значение совпадает с несколькими проверяемыми шаблоными, как поведет себя инструкция в таком случае?",
                     ...
                     optionD: "Множественное совпадение невозможно",
                     correctAnswer: 1,
                     questionId: 24,
            ...
            Question(question: ["Используем связывание значений, какой будет результат в консоли?",
                                "Что выведется в консоль при использовании связывания значений?",
            ...
            Question(question: ["Что мы получим в консоль?",
                     ...
                     correctAnswer: 1,
                     questionId: 27,
                     helpText: "В кейсе инструкции switch мы так же можем использовать дополнительное условие с помощью ключевого слова where. Три кейса объявляют константы x, y, которые временно берут значения из кортежа point. Эти константы используются как часть условия where, для создания динамического фильтра. Кейс switch совпадает с текущим значением point только в том случае, если условие оговорки where возвращает true для этого значения. Последний кейс включает в себя все возможные оставшиеся варианты, так что default не нужен, так как и без него инструкция switch является исчерпывающей."),
            ...
            Question(question: ["Сколько операторов передачи управления есть в Swift?",
                     ...
                     correctAnswer: 1,
                     questionId: 31,
                     helpText: "Операторы передачи управления меняют последовательность исполнения вашего кода, передавая управление от одного фрагмента кода другому. В Swift есть пять операторов передачи управления: continue, break, fallthrough, return, throw."),
            ...
            Question(question: ["Когда оператор управления break выполняется внутри цикла, произойдет ли следующая итерация?",
                                "Произойдет ли следующая итерация, если оператор управления break выполнится внутри цикла?",
                                "Если оператор управления break выполнится внутри цикла, начнется новая итерация, так ли это?"],
                     image: "",
                     optionA: "Нет",
                     optionB: "Да",
                     optionC: "",
            ...
            Question(question: ["Когда оператор break выполняется внутри switch-case, что произойдет?",
                                "Что произойдет, если оператор управления break выполнится внутри switch-case?",
                                "Если оператор управления break выполнится внутри switch, что произойдет дальше?"],
                     image: "",
            ...
            Question(question: ["Чтобы проигнорировать какой-либо кейс в switch, мы можем написать в теле кейса комментарий, так ли это? Если нет, что произойдет?",
                     ...
                     correctAnswer: 1,
                     questionId: 36,
                     helpText: "Кейс в инструкции switch, который содержит только комментарий, при компиляции выдаст ошибку компиляции. Комментарии - это не утверждения, и они не дают возможности игнорировать кейсы. Если вы хотите игнорировать кейс switch, используйте break."),
            
            Question(question: ["Какой оператор управления поможет вам вернуть проваливание в следующий кейс внутри switch?",
            ...
            Question(question: ["Инструкция guard выполняет код внутри скобок только тогда, когда ...?",
                                "guard: инструкция раннего выхода, выполняет код в зависимости от логического значения условия, какого?",
                                "Когда выполняется тело инструкции guard?"],
                     image: "",
                     optionA: "false",
                     optionB: "true",
                     optionC: "",
