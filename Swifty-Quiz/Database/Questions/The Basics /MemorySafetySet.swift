		...
		return [
			...
			Question(question: [
                ...
                "Swift автоматически управляет памятью?"
					 ...
					 optionC: "",
					 optionD: "",
					 questionId: 132704251,
                     helpText: """
                     Swift гарантирует, что множественный доступ к одной и той же области памяти не вызовет конфликта, так как потребует кода, который изменит местоположение в памяти, для того, чтобы у вас появился эксклюзивный доступ к этой памяти.
            ...
            Question(question: [
                "Могут ли разные части вашего кода одновременно попробовать получить доступ на запись и чтение к одному и тому же фрагменту памяти?"
                     ...
                     optionC: "",
                     optionD: "",
                     questionId: 465607562,
                     helpText: """
                     Конфликт доступа к памяти может возникнуть, когда разные части вашего кода пытаются одновременно получить доступ к одному и тому же фрагменту памяти. Множественный одновременный доступ к фрагменту памяти может привести к непредсказуемому поведению.
                     
                     В Swift существуют способы изменить значение, охватывающее несколько строк кода, что позволяет попытаться получить доступ к значению в середине его собственной модификации.
                     """),
			...
			Question(question: [
                "Сколько характеристик доступа к памяти существует в Swift?"
					 ...
					 optionC: "Одна",
					 optionD: "Четыре",
					 questionId: 569102371,
                     helpText: """
                     Существует три характеристики доступа к памяти: является ли доступ с правами чтения или записи, продолжительностью доступа и местоположением в памяти, к которой обращаются.
                     
                     В частности, конфликт может возникать, если у вас есть два доступа, отвечающие полностью всем следующим условиям: По крайней мере, один из них является доступом на запись. Они получают доступ к одному и тому же фрагменту в памяти. Их длительность перекрывается друг другом.
                     """),
			...
			Question(question: [
                ...
                "Какое различие есть между доступом на запись и доступом на чтение?"
					 ...
					 optionD: "",
					 questionId: 895154841,
                     helpText: """
                     Разница между доступом для чтения и доступом для записи очевидна: доступ на запись изменяет местоположение в памяти, а доступ для чтения нет. Местоположение в памяти относится к тому, к чему обращаются, например, к переменной, константе или свойству.
                     """),
			...
			Question(question: [
                ...
                "Выберите два корректных вида доступа к памяти в Swift"
					 ...
					 questionId: 851381194,
                     helpText: """
                     Длительность доступа к памяти - мгновенная или долгосрочная. Доступ считается мгновенным (моментальным), если невозможно запустить другой код после того, пока не завершится уже запущенный код с доступом к памяти. По своей природе два моментальных (мгновенных) доступа не могут произойти одновременно. В большинстве случаев доступ к памяти происходит мгновенно.
			...
			Question(question: [
                ...
                "Два мгновенных доступа могут произойти одновременно. Так ли это?"
					 ...
					 questionId: 95875672,
                     helpText: """
                     По своей природе два моментальных (мгновенных) доступа не могут произойти одновременно.
                     """),
			...
			Question(question: [
                ...
                "Функция имеет этот доступ для записи ко всем ее сквозным параметрам"
					 ...
					 optionC: "Обычный",
					 optionD: "",
					 questionId: 212983617,
                     helpText: """
                     Функция имеет долгосрочный доступ для записи ко всем ее сквозным параметрам. Доступ записи для сквозного параметра начинается после того, как все несквозные параметры были оценены и длится всю продолжительность вызова этой функции.
			...
			Question(question: [
                "Что мы получим после запуска данного кода?"
					 ...
					 optionD: "",
					 questionId: 586737831,
                     helpText: """
                     Одним из последствий долгосрочного доступа для записи является то, что вы не можете получить доступ к исходной переменной, которая была передана как сквозная переменная, даже если правила определения области видимости и контроля доступа это позволяют - любой доступ к оригиналу создаст конфликт.
                     
                     В приведенном примере stepSize является глобальной переменной, и она обычно доступна из increment(_:). Тем не менее, доступ для чтения к stepSize перекрывается доступом для записи к number. Доступ для чтения и записи number и stepSize относятся к одному и тому же фрагменту памяти, они перекрываются, создавая конфликт.
                     """),
			...
			Question(question: [
                "Как можно решить конфликт доступа к сквозным параметрам функции?"
					 ...
					 optionD: "",
					 questionId: 569531609,
                     helpText: """
                     Один из способов решения этого конфликта - сделать явную копию переменной, и передавать при вызове функции именно копию.
                     """),
