	...
	static func getQuestions() -> [Question] {
		return [
			...
			Question(question: ["Сколько характеристик доступа к памяти существует в Swift?"],
					 ...
					 helpText: "Существует три характеристики доступа к памяти: является ли доступ с правами чтения или записи, продолжительностью доступа и местоположением в памяти, к которой обращаются. В частности, конфликт может возникать, если у вас есть два доступа, отвечающие полностью всем следующим условиям: По крайней мере, один из них является доступом на запись. Они получают доступ к одному и тому же фрагменту в памяти. Их длительность перекрывается друг другом."),
