		...
		return [
            Question(question: [
                ...
                "Вы можете делать это с помощью контроля доступа в Swift"
                     ...
                     helpText: """
                     Контроль доступа ограничивает доступ к частям вашего кода из кода других исходных файлов и модулей.
			...
			Question(question: [
                ...
                "Члены класса с этим уровнем доступа или с более строгим уровнем доступа могут быть переопределены подклассами только в том модуле, в котором они определены"
                     ...
                     helpText: """
                     open доступ применяется только к классам и членам класса и отличается от public доступа следующим: Классы, с уровнем доступа public, могут наследоваться только в том модуле, в котором они определены.
			...
			Question(question: [
                "Что мы получим после запуска этого кода?"
                     ...
                     helpText: """
                     Пример определяет глобальную функцию someFunction, без явного указания уровня доступа самой функции. Вы, может быть, ожидаете, что эта функция будет иметь уровень доступа internal по умолчанию, но только не в нашем случае. На самом деле функция вообще не будет компилироваться.
			...
			Question(question: [
                "Если вы хотите, чтобы вложенный тип внутри public типа имел уровень доступа как public, что вам нужно сделать?"
                     ...
                     helpText: """
                     Вложенные типы, определенные внутри типа с уровнем доступа private, автоматически получают уровень доступа private. Вложенные типы внутри public типов или internal типов, автоматически получают уровень доступа как internal.
			...
			Question(question: [
                ...
                "Какой уровень доступа получают все требования протокола?"
                     ...
                     helpText: """
                     Уровень доступа каждого требования в процессе определения протокола устанавливается на тот же уровень, что и сам протокол. Вы не можете установить уровень доступа требований протокола другим, чем поддерживает сам протокол.
                     
                     Это гарантирует, что все требования протокола будут видны любому типу, который принимает протокол.
                     """),
            ...
            Question(question: [
                "Какая маркировка должна быть у модуля unit тестирования, чтобы он имел доступ к коду вашего приложения?"
                     ...
                     helpText: """
                     Когда вы пишете приложение с модулем поэлементного тестирования, то код вашего приложения должен быть доступным для модуля, чтобы он мог его проверить.
            ...
            Question(question: [
                "Какой уровень доступа будет у элементов типа в его стандартном расширении?"
                     ...
                     optionC: "public",
                     optionD: "fliprivate",
                     questionId: 977184249,
                     helpText: """
                     Вы можете расширить класс, структуру или перечисление в любом контексте, в котором класс, структура или перечисление доступны. Любой элемент типа, добавленный в расширение, имеет тот же дефолтный уровень доступа, что и типы, объявленные в исходном типе, будучи расширенными.
                     
                     Например, если вы расширяете тип public, то любые новые элементы этого типа, которые вы добавили, будут иметь уровень доступа равный internal. Если вы расширяете fileprivate тип, то дефолтный уровень доступа любого добавленного в него члена будет fileprivate. Если вы расширяете private тип, то дефолтный уровень доступа любого добавленного в него члена будет private.
                     """),
            ...
            Question(question: [
                ...
                "Можем ли мы написать расширение типа, изменив ему уровень доступа? Если да, то как?"
                     ...
                     optionD: "",
                     questionId: 693374985,
                     helpText: """
                     Вы можете отметить расширение, явно указав модификатор уровня доступа (например, private extension), для того чтобы указать новый дефолтный уровень доступа, который будут иметь элементы, определенные в этом расширении. Этот новый уровень доступа может быть переопределен для отдельных элементов расширением.
                     """),
