...
class InitializationSet {
	static func getQuestions() -> [Question] {
		return [
			...
			Question(question: [
                "Если вы не укажите своё внешнее имя для параметра инициализатора, что произойдет?",
                "Если вы не указали внешнее имя для параметра инициализатора, произойдет это"
					 ...
					 optionD: "",
					 questionId: 126029946,
					 helpText: "Имя предоставляется автоматически. Swift предоставляет автоматические внешние имена для каждого параметра, если вы, конечно, не укажите своё внешнее имя."),
			...
			Question(question: [
                ...
                "Какой результат мы получим после запуска данного кода?"
                     ...
                     (Свойство name - свойство опционального типа String, значит значение по умолчанию равно nil).
			...
			Question(question: [
                "Сокращенный способ инициализировать свойства нового экземпляра структуры",
                "При инициализации нового экземпляра структуры, мы можем воспользоваться этим инициализатором"
					 ...
					 questionId: 757192982,
					 helpText: "Почленный инициализатор. Почленный инициализатор - это сокращенный способ инициализировать свойства нового экземпляра структуры. Начальные значения для свойств нового экземпляра могут быть переданы в почленный инициализатор по имени."),
			...
			Question(question: [
                "Как записываются назначенные инициализаторы классов?",
			...
			Question(question: [
                ...
                "Вспомогательные инициализаторы для классов записываются именно так"
					 ...
					 optionC: "init() { }",
					 optionD: "override init() { }",
					 questionId: 905287111,
                     helpText: """
                     convenience init() { }. Вспомогательные инициализаторы пишутся как назначенные, но только дополнительно используется вспомогательное слово convenience, которое располагается до слова init и разделяется пробелом.
			...
			Question(question: [
                "Обязаны ли мы обеспечивать вспомогательные инициализаторы, если класс не нуждается в них?",
                "Если наш класс не нуждается во вспомогательных инициализаторах, обязаны ли мы их обеспечить?"
					 ...
					 optionC: "",
					 optionD: "",
					 questionId: 731762124,
					 helpText: "Нет. Вы не обязаны обеспечивать вспомогательные инициализаторы, если ваш класс не нуждается в них. Создавайте вспомогательный инициализатор всякий раз, когда это является наиболее рациональным путем общей инициализации и может сэкономить время и сделать саму инициализацию класса более чистой и краткой."),
			
			Question(question: [
                "Как делегируют назначенные инициализаторы?",
			...
			Question(question: [
                ...
                "Сколько назначенных инициализаторов имеет подкласс?"
					 ...
					 optionC: "3",
					 optionD: "Нисколько",
					 questionId: 301695592,
                     helpText: """
                     2. Здесь, в суперклассе есть один назначенный инициализатор и два вспомогательных инициализатора. Один вспомогательный инициализатор вызывает другой вспомогательный инициализатор, который в свою очередь вызывает единственный назначенный инициализатор.
			...
			Question(question: [
                ...
                "Чтобы убедиться, что двухфазная инициализация прошла без ошибок, Swift проводит такое количество проверок (каждый раз)"
                     ...
                     Проверка 3: Вспомогательный инициализатор должен делегировать другому инициализатору до того, как будут присвоены значения любым свойствам (включая свойства определенные тем же классом).
			...
			Question(question: [
                "Согласно первой проверке безопасности двухфазной инициализации в Swift, память считается полностью инициализированной только для этих объектов"
                     ...
                     Для того чтобы удовлетворить этому правилу, назначенный инициализатор должен убедиться, что все его собственные свойства инициализированы до того, как будут переданы вверх по цепочке.
                     """),
			
			Question(question: [
                "Если назначенный инициализатор не делегирует суперклассу инициализатор до присваивания значений унаследованным свойствам, что произойдет со значениями, которые он присвоил?"
            ],
					 image: "",
					 ...
					 optionD: "",
					 questionId: 595402365,
                     helpText: """
                     Перепишутся суперклассом. Назначенный инициализатор должен делегировать суперклассу инициализатор до присваивания значений унаследованным свойствам.
			...
			Question(question: [
                "Если вспомогательный инициализатор не делегировал другому инициализатору до того, как были присвоены значения любым свойствам, что произойдет со значениями свойств?"
            ],
					 image: "",
					 ...
					 questionId: 653118106,
                     helpText: """
                     Перезапишутся назначенным. Вспомогательный инициализатор должен делегировать другому инициализатору до того, как будут присвоены значения любым свойствам (включая свойства определенные тем же классом).
                     
                     Если этого сделано не будет, то новое значение, которое присваивает вспомогательный инициализатор, будет перезаписано его собственным назначенным инициализатором класса.
                     """),
			...
			Question(question: [
                ...
                "К какой фазе инициализации относятся эти пункты: \"Назначенный инициализатор для этого класса подтверждает, что все свойства, представленные этим классом, имеют значения\"?"
					 ...
					 optionD: "Ни к какой",
					 questionId: 163900124,
                     helpText: """
                     К первой фазе. Фаза первая: Назначенный или вспомогательный инициализатор вызывается в классе. Память под новый экземпляр этого класса выделяется, но она еще не инициализирована.
                     
                     Назначенный инициализатор для этого класса подтверждает, что все свойства, представленные этим классом, имеют значения. Память под эти свойства теперь инициализирована. Назначенный инициализатор передает инициализатору суперкласса, что пора проводить те же действия, только для его собственных свойств. Так продолжается по цепочке до самого верхнего суперкласса.
			...
			Question(question: [
                ...
                "К какой фазе инициализации относится этот пункт: \"Теперь инициализаторы получают доступ к self и могут изменять свои свойства, создавать экземпляры и вызывать методы и т.д.\"?"
					 ...
					 optionC: "К третьей фазе",
					 optionD: "Ни к какой",
					 questionId: 174250220,
                     helpText: """
                     Ко второй фазе. Фаза вторая: Двигаясь вниз по цепочке, каждый назначенный инициализатор в этой цепочке имеет такую возможность, как настраивать экземпляр.
                     
                     Теперь инициализаторы получают доступ к self и могут изменять свои свойства, создавать экземпляры и вызывать методы и т.д. И наконец, каждый вспомогательный инициализатор в цепочки имеет возможность настраивать экземпляр и работать с self.
                     """),
			...
			Question(question: [
                ...
                "На изображении эта фаза инициализации для гипотетического подкласса и суперкласса"
					 ...
					 optionC: "Третья",
					 optionD: "Четвертая",
					 questionId: 827385202,
                     helpText: """
                     Первая. Так выглядит первая фаза инициализации для гипотетического подкласса и суперкласса: Инициализация начинается с вызова вспомогательного инициализатора подкласса.
                     
                     Вспомогательный инициализатор пока не может изменять каких-либо свойств. Он делегирует назначенному инициализатору по тому же классу, где и он сам. Назначенный инициализатор убеждается, что все свойства, подкласса имеют значения.
                     
                     После этого он вызывает назначенный инициализатор своего суперкласса для продолжения. Назначенный инициализатор суперкласса проверяет наличие значений у всех свойств суперкласса. Сразу после того как все свойства суперкласса получают начальные значения, память считается полностью инициализированной, Фаза 1 завершается.
                     """),
			...
			Question(question: [
                ...
                "На изображении эта фаза инициализации для гипотетического подкласса и суперкласса"
					 ...
					 optionC: "Третья",
					 optionD: "Четвертая",
					 questionId: 200872205,
                     helpText: """
                     Вторая. Фаза №2. Назначенный инициализатор суперкласса только теперь получает возможность менять дальнейшие экземпляры(хотя и не обязан).
                     ...
                     Наконец, как только заканчивает работу назначенный инициализатор подкласса, то возможность вносить изменения получает вспомогательный инициализатор, который изначально был вызван. То есть если рассмотреть весь процесс отдаленно, то он получается, как бы, идет вверх, а потом спускается вниз.
                     """),
			...
			Question(question: [
                ...
                "Этим модификатором мы должны помечать инициализатор суперкласса внутри подкласса (переопределение)"
                     ...
                     Когда вы пишете инициализатор подкласса, который совпадает с назначенным инициализатором суперкласса, вы фактически переопределяете назначенный инициализатор. Таким образом вы должны писать модификатор override перед определением инициализатора подкласса.
                     
                     Это верно даже если вы переопределяете автоматически предоставляемый инициализатор.
                     """),
			
			Question(question: [
                "Должны ли мы использовать модификатор override, если написали инициализатор подкласса, который совпадает с вспомогательным инициализатором суперкласса?",
                "Если наш инициализатор подкласса совпадает со вспомогательным инициализатором суперкласса, должны ли мы использовать модификатор override?"
					 ...
					 optionD: "",
					 questionId: 187078387,
                     helpText: """
                     Нет. И наоборот, если вы пишете инициализатор подкласса, который совпадает с вспомогательным инициализатором суперкласса, то этот вспомогательный инициализатор суперкласса никогда не сможет быть вызван напрямую вашим подклассом, в соответствии с правилами указанными выше.
			...
			Question(question: [
                "Будет ли подкласс наследовать все назначенные инициализаторы суперкласса, если сам не определяет ни одного?",
                "Ваш подкласс будет наследовать все назначенные инициализаторы суперкласса, если сам не определяет ни одного назначенного инициализатора. Так ли это?"
					 ...
					 optionC: "",
					 optionD: "",
					 questionId: 586996644,
					 helpText: "Да. Если ваш подкласс не определяет ни одного назначенного инициализатора, он автоматически наследует все назначенные инициализаторы суперкласса."),
			
			Question(question: [
                "Если у класса есть реализация всех назначенных инициализаторов его суперкласса, будет ли он автоматически наследовать все вспомогательные инициализаторы этого суперкласса?"
            ],
					 image: "",
					 optionA: "Да",
					 optionB: "Нет",
					 optionC: "",
					 optionD: "",
					 questionId: 431314243,
                     helpText: """
                     Да. Если у вашего класса есть реализация всех назначенных инициализаторов его суперкласса, либо они были унаследованы (если ваш подкласс не определяет ни одного назначенного инициализатора, он автоматически наследует все назначенные инициализаторы суперкласса) или же представлены как часть пользовательской реализации определения подкласса, то тогда этот подкласс автоматически наследует все вспомогательные инициализаторы суперкласса.
                     """),
			...
			Question(question: [
                ...
                "Иногда бывает нужно определить класс, структуру или перечисление, инициализация которого может не сработать (такое неисполнение, например, может быть вызвано некорректными значениями параметров). Что мы можем использовать?"
					 ...
					 questionId: 559107874,
                     helpText: """
                     Проваливающийся инициализатор. Иногда бывает нужно определить класс, структуру или перечисление, инициализация которого может не сработать, провалиться.
			...
			Question(question: [
                "Можем ли мы определить проваливающийся инициализатор и обычные инициализаторы с одними и теми же именами и типами параметров?",
                "Мы можем определить проваливающийся инициализатор и обычные инициализаторы с одними и теми же именами и типами параметров, так ли это?"
					 ...
					 optionC: "Иногда",
					 optionD: "",
					 questionId: 605878210,
                     helpText: """
                     Нет. Вы не можете определить проваливающийся инициализатор и обычные инициализаторы с одними и теми же именами и типами параметров. Проваливающийся инициализатор создает опциональное значение типа, который он инициализирует.
                     
                     Вы пишете return nil внутри проваливающегося инициализатора для индикации точки, где инициализация может провалиться.
                     """),
			
			Question(question: [
                "Как пишется проваливающийся инициализатор?"
            ],
					 image: "",
					 optionA: "init?() { }",
					 optionB: "init()? { }",
                     ...
                     Вы можете написать проваливающийся инициализатор поместив вопросительный знак после ключевого слова init (init?).
                     """),
			
			Question(question: [
                "Можем ли мы использовать проваливающийся инициализатор для перечислений?",
                "Вы можете использовать проваливающийся инициализатор для выбора подходящего члена перечисления, основываясь на одном или более параметров. Так ли это?"
					 ...
					 optionC: "",
					 optionD: "",
					 questionId: 875904274,
					 helpText: "Да. Вы можете использовать проваливающийся инициализатор для выбора подходящего члена перечисления основываясь на одном или более параметров. Инициализатор может провалиться, если предоставленные параметры не будут соответствовать подходящему члену перечисления."),
			...
			Question(question: [
                "Может ли проваливающийся инициализатор класса, структуры, или перечисления быть делегирован другому проваливающемуся инициализатору из того же класса?",
			...
			Question(question: [
                "Какой инициализатор мы будем использовать, когда нам нужно, чтобы каждый подкласс этого класса был обязан реализовать его?"
					 ...
					 optionC: "Вспомогательный",
					 optionD: "Почленный",
					 questionId: 132251335,
					 helpText: "Требуемый. Напишите required перед определением инициализатора класса, если вы хотите, чтобы каждый подкласс этого класса был обязан реализовывать этот инициализатор."),
			...
			Question(question: [
                "Мы обязаны писать override каждый раз, когда переопределяем требуемый инициализатор. Это так?",
                "Обязаны ли мы писать override всегда, когда переопределяем требуемый инициализатор суперкласса?"
					 ...
					 optionC: "",
					 optionD: "",
					 questionId: 350209500,
					 helpText: "Нет. Вы также должны писать модификатор required перед каждой реализацией требуемого инициализатора класса для индикации того, что последующий подкласс так же должен унаследовать этот инициализатор по цепочке. Вы не пишете override, когда переопределяете требуемый инициализатор."),
			
			Question(question: [
                "Обязаны ли мы обеспечивать явную реализацию требуемого инициализатора в подклассах, если нам достаточно унаследованного?",
                "Если мы можем удовлетворить требования инициализации унаследованным инициализатором суперкласса, должны ли мы явно реализовывать требуемый инициализатор в нашем подклассе?"
					 ...
					 optionC: "",
					 optionD: "",
					 questionId: 300682311,
					 helpText: "Нет. Вы не должны обеспечивать явную реализацию требуемого инициализатора, если вы можете удовлетворить требование унаследованным инициализатором."),
			
			Question(question: [
                "Если начальное значение свойства требует какой-то настройки или структуризации, можете ли вы использовать замыкание?",
                "Если начальное значение свойства требует какой-то структуризации или настройки, можете ли вы использовать глобальную функцию?"
					 ...
					 optionC: "",
					 optionD: "",
					 questionId: 387118598,
                     helpText: """
                     Да. Если начальное значение свойства требует какой-то настройки или структуризации, то вы можете использовать замыкание или глобальную функцию, которая будет предоставлять значение для этого свойства.
                     
                     Как только создается новый экземпляр, вызывается функция или замыкание, которая возвращает значение, которое присваивается в качестве начального значения свойства.
