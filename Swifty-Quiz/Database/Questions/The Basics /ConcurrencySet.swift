...
class ConcurrencySet {
    static func getQuestions() -> [Question] {
        return [
            ...
            Question(question: ["Особый вид функции, которую можно приостановить на полпути выполнения"],
                     image: "",
                     optionA: "Асинхронная функция",
                     optionB: "Сбегающая функция",
                     optionC: "Синхронная функция",
                     optionD: "",
                     questionId: 158823891,
                     helpText: "Асинхронная функция или асинхронный метод - это особый вид функции или метода, которые можно приостановить на полпути выполнения. Это отличается от обычных синхронных функций и методов, которые либо выполняются до завершения, либо вызывают ошибку, либо никогда не возвращаются. Асинхронная функция или метод по-прежнему выполняет одно из этих трех действий, но может также останавливаться посередине, когда чего-то ожидает. Внутри тела асинхронной функции или метода вы отмечаете каждое из этих мест, где выполнение может быть приостановлено."),
            
            Question(question: ["Может ли асинхронная функция выполнять какие-либо действия, и при этом останавливаться посередине выполнения, когда чего-то ожидает?"],
                     image: "",
                     optionA: "Да",
                     optionB: "Нет",
                     optionC: "",
                     optionD: "",
                     questionId: 845423929,
                     helpText: "Асинхронная функция или метод по-прежнему выполняет одно из этих трех действий, но может также останавливаться посередине, когда чего-то ожидает. Внутри тела асинхронной функции или метода вы отмечаете каждое из этих мест, где выполнение может быть приостановлено."),
            
            Question(question: ["Если асинхронная функция может быть приостановлена в нескольких местах, что мы должны сделать внутри тела этой функции?"],
                     image: "",
                     optionA: "Отметить точки приостановки",
                     optionB: "Поставить брейкпоинты",
                     optionC: "Залогировать эти места",
                     optionD: "",
                     questionId: 183642495,
                     helpText: "Асинхронная функция или метод по-прежнему выполняет одно из этих трех действий, но может также останавливаться посередине, когда чего-то ожидает. Внутри тела асинхронной функции или метода вы отмечаете каждое из этих мест, где выполнение может быть приостановлено."),
            
            Question(question: ["Какой ключевое слово мы добавляем функции, чтобы указать, что она является асинхронной?"],
                     image: "",
                     optionA: "async",
                     optionB: "await",
                     optionC: "global",
                     optionD: "sync",
                     questionId: 755689114,
                     helpText: "Чтобы указать, что функция или метод является асинхронным, вы пишете ключевое слово async в его объявлении после его параметров, аналогично тому, как вы используете throw для отметки функции с исключением. Если функция или метод возвращает значение, вы пишете async перед стрелкой возврата (->)."),
            
            Question(question: ["Где мы должны писать ключевое слово async относительно стрелки возврата ->, если наша асинхронная функция возвращает значение?"],
                     image: "",
                     optionA: "async ->",
                     optionB: "-> async",
                     optionC: "",
                     optionD: "",
                     questionId: 869903712,
                     helpText: "Если функция или метод возвращает значение, вы пишете async перед стрелкой возврата (->)."),
            
            Question(question: ["Если метод является как асинхронным, так и исключающим, где мы пишем ключевое слово async?"],
                     image: "",
                     optionA: "async перед throw",
                     optionB: "async после throw",
                     optionC: "Не принципиально",
                     optionD: "",
                     questionId: 876599134,
                     helpText: "Для функции или метода, которые одновременно являются как асинхронными, так и исключающими (throw), вы пишете async перед throw."),
            
            Question(question: ["Каким ключевым словом помечается точка приостановки внутри асинхронной функции?"],
                     image: "",
                     optionA: "await",
                     optionB: "try",
                     optionC: "async",
                     optionD: "throw",
                     questionId: 276242571,
                     helpText: "При вызове асинхронного метода выполнение приостанавливается до тех пор, пока этот метод не вернется. Вы пишете await перед вызовом, чтобы отметить возможную точку приостановки. Это похоже на запись try при вызове бросающей функции, чтобы отметить возможное изменение потока программы в случае ошибки. Внутри асинхронного метода поток выполнения приостанавливается только тогда, когда вы вызываете другой асинхронный метод - приостановка никогда не бывает неявной или упреждающей - это означает, что каждая возможная точка приостановки помечается с помощью await."),
            
            Question(question: ["При уступке потока, Swift приостанавливает выполнение вашего кода в текущем потоке и вместо этого запускает другой код. Он делает это в этом же потоке, или в другом?"],
                     image: "",
                     optionA: "В этом же потоке",
                     optionB: "В другом потоке",
                     optionC: "",
                     optionD: "",
                     questionId: 415277905,
                     helpText: "Возможные точки приостановки в вашем коде, отмеченные значком await, указывают на то, что текущий фрагмент кода может приостановить выполнение, ожидая возврата асинхронной функции или метода. Это также называется уступкой потока, потому что за кулисами Swift приостанавливает выполнение вашего кода в текущем потоке и вместо этого запускает другой код в этом потоке."),
            
            Question(question: ["Можем ли мы вызывать асинхронные функции или методы в теле другой асинхронной функции, метода, или свойства?"],
                     image: "",
                     optionA: "Да",
                     optionB: "Нет",
                     optionC: "Только внутри свойства",
                     optionD: "",
                     questionId: 390561061,
                     helpText: "Поскольку код с await должен иметь возможность приостанавливать выполнение, только определенные места в вашей программе могут вызывать асинхронные функции или методы: Код в теле асинхронной функции, метода или свойства; Код в статическом методе main() структуры, класса или перечисления, помеченных @main; Код в отдельной дочерней задаче."),
            
            Question(question: ["Этот цикл приостанавливает выполнение в начале каждой итерации"],
                     image: "",
                     optionA: "for-await-in",
                     optionB: "try-for-in",
                     optionC: "for-in",
                     optionD: "async-for-in",
                     questionId: 846248831,
                     helpText: "Цикл for-await-in потенциально приостанавливает выполнение в начале каждой итерации, когда он ожидает, когда будет доступен следующий элемент. Точно так же, как вы можете использовать свои собственные типы в цикле for-in, добавив соответствие протоколу Sequence, вы можете использовать свои собственные типы в цикле for-await-in, добавив соответствие протоколу AsyncSequence."),
            
            Question(question: ["Чтобы использовать свои кастомные типы внутри цикла for-await-in, соответствие какому протоколу мы должны присвоить этому циклу?"],
                     image: "",
                     optionA: "AsyncSequence",
                     optionB: "Async",
                     optionC: "Sequence",
                     optionD: "SyncSequence",
                     questionId: 428951482,
                     helpText: "Точно так же, как вы можете использовать свои собственные типы в цикле for-in, добавив соответствие протоколу Sequence, вы можете использовать свои собственные типы в цикле for-await-in, добавив соответствие протоколу AsyncSequence."),
            
            Question(question: ["Целесообразно ли используется асинхронность в этом примере?"],
                     image: "Concurency01",
                     optionA: "Нет",
                     optionB: "Да",
                     optionC: "",
                     optionD: "",
                     questionId: 688777639,
                     helpText: "Вызов асинхронной функции с помощью await запускает только один фрагмент кода за раз. Пока выполняется асинхронный код, вызывающая сторона ожидает завершения этого кода, прежде чем перейти к выполнению следующей строки кода. Например, чтобы получить первые три фотографии из галереи, вы можете дождаться трех вызовов функции downloadPhoto(named :). У этого подхода есть важный недостаток: несмотря на то, что загрузка является асинхронной и позволяет выполнять другую работу во время ее выполнения, одновременно выполняется только один вызов функции downloadPhoto(named :). Каждая фотография полностью загружается до начала загрузки следующей. Однако этим операциям не нужно ждать - каждую фотографию можно загружать независимо или даже одновременно."),
            
            Question(question: ["В данном примере все три вызова функции download(named:) запускаются ___. Как?"],
                     image: "Concurency02",
                     optionA: "Одновременно, без ожидания",
                     optionB: "Последовательно, по очереди",
                     optionC: "При запуске будет ошибка",
                     optionD: "",
                     questionId: 759040431,
                     helpText: "Чтобы вызвать асинхронную функцию и позволить ей работать параллельно с кодом вокруг нее, напишите async перед let при определении константы, а затем напишите await каждый раз, когда вы используете константу. В этом примере все три вызова downloadPhoto(named :) запускаются без ожидания завершения предыдущего. Если доступно достаточно системных ресурсов, они могут работать одновременно. Ни один из этих вызовов функций не помечен как await, потому что код не приостанавливается в ожидании результата функции. Вместо этого выполнение продолжается до тех пор, пока не будет определена строка, в которой определены фотографии - в этот момент программе требуются результаты этих асинхронных вызовов, поэтому вы пишете await, чтобы приостановить выполнение, пока не завершится загрузка всех трех фотографий."),
            
            Question(question: ["Единица работы, которая может выполняться асинхронно как часть вашей программы"],
                     image: "",
                     optionA: "Задача",
                     optionB: "Вызов",
                     optionC: "Группа задач",
                     optionD: "Операнд",
                     questionId: 526160507,
                     helpText: "Задача - это единица работы, которая может выполняться асинхронно как часть вашей программы. Весь асинхронный код выполняется как часть некоторой задачи. Синтаксис async-let, описанный в предыдущем разделе, создает для вас дочернюю задачу. Вы также можете создать группу задач и добавить в нее дочерние задачи, что дает вам больше контроля над приоритетом и отменой, а также позволяет создавать динамическое количество задач."),
            
            Question(question: ["Как называется подход, когда задачи расположены в иерархии. Каждая задача в группе задач имеет одну и ту же родительскую задачу, и каждая задача может иметь дочерние задачи"],
                     image: "",
                     optionA: "Структурированный параллелизм",
                     optionB: "Неструктурированный параллелизм",
                     optionC: "Запараллеливание",
                     optionD: "",
                     questionId: 147539692,
                     helpText: "Задачи расположены в иерархии. Каждая задача в группе задач имеет одну и ту же родительскую задачу, и каждая задача может иметь дочерние задачи. Из-за явной взаимосвязи между задачами и группами задач этот подход называется структурированным параллелизмом. Хотя вы берете на себя часть ответственности за правильность, явные родительско-дочерние отношения между задачами позволяют Swift обрабатывать некоторые действия, такие как распространение отмены, и позволяет Swift обнаруживать некоторые ошибки во время компиляции."),
            
            Question(question: ["Акторы являются ссылочным типом или типом значения?"],
                     image: "",
                     optionA: "Ссылочный тип",
                     optionB: "Тип значения",
                     optionC: "Оба ответа неверны",
                     optionD: "",
                     questionId: 583522085,
                     helpText: "Как и классы, акторы являются ссылочными типами, поэтому сравнение типов значений и ссылочных типов в разделе «Классы являются ссылочными типами» применяется как к акторам, так и к классам. В отличие от классов, акторы позволяют только одной задаче получать доступ к своему изменяемому состоянию за раз, что делает безопасным взаимодействие кода в нескольких задачах с одним и тем же экземпляром актора."),
        ]
    }
}
