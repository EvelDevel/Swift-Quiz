    ...
    static func getQuestions() -> [Question] {
        return [
            Question(question: ["В каком варианте создается словарь?"],
                     ...
                     helpText: "Словарь создается в первом варианте. У нас есть ключ типа String и значение типа String. Второй вариант создает массив из двух элементов типа String."),
            ...
            Question(question: ["Скомпилируется ли этот код? Если да, то какое значение мы получим в earth?"],
                     ...
                     helpText: "Код является корректным, мы получим значение Earth. Если мы пытаемся получить значение словаря по ключу, которого в нем нет (в нашем случае, например: 2) - мы получим nil. Мы можем исправить это, добавив значение по умолчанию в момент обращения к словарю по определенному ключу, чтобы, когда мы получаем nil, возвращалось default значение."),
            ...
            Question(question: ["Скомпилируется ли этот код? Если да, то какое значение мы получим в переменной earth?"],
                     ...
                     helpText: "Код скомпилируется и мы получим значение Home. Если мы пытаемся получить значение словаря по ключу, которого в нем нет (как в нашем случае), мы получим nil. Мы можем исправить это, добавив значение по умолчанию в момент обращения к словарю по определенному ключу, чтобы, когда мы получаем nil, возвращалось default значение. Это как раз то, что произойдет в нашем случае, когда мы вернем дефолтное значение вместо отсутствующего."),
            ...
            Question(question: ["Что мы получим послез запуска этого кода?"],
                     ...
                     helpText: "В нашем словаре numbers ключи имеют тип Int. Когда мы создаем константу one, мы обращаемся к словарю по ключу с типом String, и так как в нашем словаре у ключей другой тип, мы получим ошибку компиляции (No exact matches in call to subscript)."),
        ]
    }
}
