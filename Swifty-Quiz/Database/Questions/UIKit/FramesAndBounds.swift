    ...
    static func getQuestions() -> [Question] {
        return [
            ...
            Question(question: ["Что мы обязательно должны задать для frame нашего view, помимо его исходной точки (origin)?"],
                     ...
                     helpText: "View отвечает за рисование и обработку событий в прямоугольной области окна. Чтобы указать этот прямоугольник \"ответственности\", вы определяете его местоположение как исходную точку (origin) и размер (width / height), используя систему координат."),
            ...
            Question(question: ["Какой тип имеет origin у frame?"],
                     ...
                     helpText: "origin описывает координаты начальной точки для frame или bounds. Она имеет тип CGPoint, и содержит в себе два параметра, x - y."),
            ...
            Question(question: ["Какой тип имеет frame.size?"],
                     ...
                     helpText: ".size описывает высоту и ширину нашего прямоугольника, имеет тип CGSize (структура, содержащая значение высоты и значение ширины, height / width)."),
            ...
            Question(question: ["Будет ли вызываться метод draw(_:) при простом изменении frame?"],
                     ...
                     helpText: "При изменении прямоугольника frame, view автоматически переотобразится без вызова метода draw(_:). Если вы хотите, чтобы UIKit вызывал метод draw(_:) при изменении прямоугольника frame, установите для свойства contentMode значение UIView.ContentMode.redraw."),
            ...
            Question(question: ["Каким образом мы можем корректно изменить положение view после того, как применили к нему масштабирование или вращение через transform?"],
                     ...
                     helpText: "Центральная точка (.center) указывается в поинтах (points) в системе координат superview. Установка этого свойства соответствующим образом обновляет исходную точку прямоугольника в свойстве frame. Используйте это свойство вместо свойства frame, если вы хотите изменить положение view. Центральная точка всегда валидна (корректно расположена), даже если к view применялось масштабирование или вращение (через transform). Изменения этого свойства можно анимировать."),
            ...
            Question(question: ["Будут ли отличаться параметры bounds для этого view на изображениях слева и справа (зеленая рамка)?"],
                     ...
                     helpText: "Если мы повернем view на 20 градусов по часовой стрелке (вращение выполняется с помощью преобразования, описанного в документации, через transform) - bounds останется прежним (origin 0, 0), он не знает что произошло c frame. Однако, все значения frame изменились."),
            ...
            Question(question: ["Структура, содержащая расположение и размеры прямоугольника"],
                     ...
                     helpText: "CGRect это структура, содержащая расположение и размеры прямоугольника."),
            ...
            Question(question: ["Следует ли нам использовать значения frame, после преобразования нашего view через transform?"],
                     ...
                     helpText: "Если вы трансформируете view, frame становится неопределенным. Это означает, что если вы поворачиваете, масштабируете или выполняете какое-либо другое преобразование вашего view через transform, вам больше не следует использовать значения frame, но вы можете продолжать использовать значения bounds."),
            ...
            Question(question: ["Мы создаем frame для нашего view используя CGRect. У него есть возможность инициализации как с помощью Int значений, так и с помощью CGFloat значений. Это так?"],
                     ...
                     helpText: "У CGRect есть четыре инициализатора: 1. init(origin: CGPoint, size: CGSize) - создает прямоугольник с указанием origin точки и размеров; 2. init(x: Double, y: Double, width: Double, height: Double) - создает прямоугольник, принимая на вход числа с плавающей точкой; 3. init(x: Int, y: Int, width: Int, height: Int) - создает прямоугольник, используя целые числа; 4. init(x: CGFloat, y: CGFloat, width: CGFloat, height: CGFloat) - создает прямоугольник, используя CGFloat значения."),
            ...
            Question(question: ["Как мы можем получить размер view, если мы его трансформировали через transform?"],
                     ...
                     helpText: "Используйте bounds, когда вы вносите внутренние изменения, например, рисуете объекты или упорядочиваете subviews в вашем view. Также используйте bounds, чтобы получить размер вашего view, если вы его изменяли через transform."),
            ...
            Question(question: ["Можем ли мы создать CGRect для нашего frame используя CGFloat значения?"],
                     ...
                     helpText: "Конечно можем, у CGRect есть четыре инициализатора: 1. init(origin: CGPoint, size: CGSize) - создает прямоугольник с указанием origin точки и размеров; 2. init(x: Double, y: Double, width: Double, height: Double) - создает прямоугольник, принимая на вход числа с плавающей точкой; 3. init(x: Int, y: Int, width: Int, height: Int) - создает прямоугольник, используя целые числа; 4. init(x: CGFloat, y: CGFloat, width: CGFloat, height: CGFloat) - создает прямоугольник, используя CGFloat значения."),
            ...
            Question(question: ["Мы можем создать frame с нулевыми значениями (origin (0,0) и размером (0,0)) через пустой инициализатор init(). Каким еще способом мы можем создать подобный frame?"],
                     ...
                     helpText: "Прямоугольник со значениями 0 можно создать двумя способами: 1. Через статическое свойство .zero (static var zero: CGRect) - cоздает прямоугольник, у которого origin и размер равны нулю. 2. Через пустую инициализацию - init() - создаст прямоугольник с origin (0,0) и размером (0,0)."),
        ]
    }
}
