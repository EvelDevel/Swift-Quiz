...
class AutoLayout {
    static func getQuestions() -> [Question] {
        return [
            ...
            Question(question: ["Есть ли такие разновидности view в Swift, у которых есть свой минимальный размер без ограничений?"],
                     image: "",
                     optionA: "Да",
                     optionB: "Нет",
                     optionC: "",
                     ...
                     helpText: "Некоторые view имеют естественный размер, опирающийся на их внутренний контент. Это называется их внутренним размером содержимого (intrinsic content size). Например, внутренний размер содержимого кнопки - это размер ее заголовка (title) плюс небольшие отступы (margin). Не все view имеют внутренний размер содержимого (intrinsic size). Для view, у которых этот размер есть, он может определять высоту view, его ширину или и то, и другое."),
            ...
            Question(question: ["Для view, у которых есть внутренний размер содержимого (intrinsic content size), он может определять высоту, ширину или и то, и другое. Правда ли это?"],
                     ...
                     helpText: "Некоторые view имеют естественный размер, опирающийся на их внутренний контент. Это называется их внутренним размером содержимого (intrinsic content size). Например, внутренний размер содержимого кнопки - это размер ее заголовка (title) плюс небольшие отступы (margin). Не все view имеют внутренний размер содержимого (intrinsic content size). Для view, у которых этот размер есть, он может определять высоту view, его ширину или и то, и другое."),
            ...
            Question(question: ["Выберите, для каких нативных элементов intrinsic content size (внутренний размер содержимого) может влиять и на ширину, и на высоту элемента"],
                     ...
                     helpText: "Внутренний размер содержимого (intrinsic content size) может определять и высоту и ширину для следующих элементов: Labels, buttons, switches, and text fields. UIView and NSView не имеют внутреннего размера, влияющего на внешний. Внутренний размер для Sliders (слайдер) - может влиять только на ширину, и Text views с image views - вариативно."),
            ...
            Question(question: ["Выберите, для каких нативных элементов intrinsic content size (внутренний размер содержимого) может влиять и на ширину, и на высоту элемента"],
                     ...
                     helpText: "Внутренний размер содержимого (intrinsic content size) может определять и высоту и ширину для следующих элементов: Labels, buttons, switches, and text fields. UIView and NSView не имеют внутреннего размера, влияющего на внешний. Внутренний размер для Sliders (слайдер) - может влиять только на ширину, и для Text views с image views - view пустого изображения не имеет внутреннего размера содержимого. Однако как только вы добавляете изображение, его внутренний размер содержимого устанавливается равным размеру изображения."),
            ...
            Question(question: ["Есть ли у пустого Image view свой внутренний размер содержимого (intrinsic content size)?"],
                     ...
                     helpText: "Для Text views с image views - view без текста или изображения не имеет внутреннего размера содержимого. Однако как только вы добавляете изображение, его внутренний размер содержимого устанавливается равным размеру изображения."),
            ...
            Question(question: ["При включенном скролле text view не имеет внутреннего размера содержимого. Правда ли это?"],
                     ...
                     helpText: "Внутренний размер содержимого text view зависит от контента (текста), от того, включен ли в нем скролл, а также от других ограничений, применяемых к нему. Например, при включенном скролле text view не имеет внутреннего размера содержимого. Если скролл отключен, по умолчанию внутренний размер содержимого text view рассчитывается на основе размера текста без переноса строк. Например, если в тексте нет возврата (\n), он вычисляет высоту и ширину, необходимые для размещения содержимого в виде одной строки текста. Если вы добавляете ограничения для указания ширины представления, внутренний размер содержимого определяет высоту, необходимую для отображения текста с учетом его ширины."),
            ...
            Question(question: ["Content hugging priorities отталкивают края view или наоборот притягивает их к содержимому?"],
                     ...
                     helpText: "Auto Layout представляет внутренний размер содержимого view с использованием пары ограничений для каждого измерения (вертикального и горизонтального). Обхват содержимого (content hugging priorities) притягивает края view внутрь, так чтобы они плотно прилегали к содержимому. Сопротивление сжатию (compression resistance priorities) отталкивает края view наружу, чтобы контент нигде не был \"обрезан\"."),
