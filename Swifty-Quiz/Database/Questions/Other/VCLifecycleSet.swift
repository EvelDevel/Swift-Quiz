    ...
    static func getQuestions() -> [Question] {
        return [
            ...
            Question(question: ["Если ваш класс не является наследником UIViewController, будут ли вам доступны методы жизненного цикла вью контроллера?"],
                     ...
                     helpText: "Чтобы получить доступ ко всем стандартным методам вью контроллера, необходимо, чтобы ваш класс наследовался от UIViewController, и в некоторых переопределенных методах обращался к суперклассу super."),
            ...
            Question(question: ["Можем ли мы обратиться к view нашего вью контроллера из awakeFromNib или init(nibName:bundle:)?",
                     ...
                     helpText: "К моменту awakeFromNib, в случае загрузки из сториборда, или init(nibName:bundle:) в случае программного создания вью контроллера, наш экран находится в состоянии Unloaded, то есть его интерфейс еще не загружен, важно знать, что в этот момент графические элементы интерфейса еще не созданы, а ссылки на объекты, которые должны быть доступны через IBOutlets и IBActions еще не назначены и при попытке обратиться к ним мы столкнемся с ошибкой, и ваше приложение упадет (Fatal error)."),
            ...
            Question(question: ["Если внутри вашего вью контроллера вы установите основному view свойство isHidden = true, будет ли вызван метод viewWillAppear?"],
                     ...
                     helpText: "Даже если вы скрыли основное вью внутри вью контроллера (присвоили его свойству isHidden = true), и оно в данный момент не отображается на экране (или скрыли любое другое вью), дефолтный метод viewWillAppear все равно будет вызван, так как с помощью него скрытое вью добавляется в иерархию вью (view hierarchy)."),
