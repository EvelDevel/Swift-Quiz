    ...
    static func getQuestions() -> [Question] {
        return [
            ...
            Question(question: ["Можем ли мы внедрить Сервис локатор как зависимость Constructor Injection, чтобы потом обращаться к нему внутри класса?"],
                     ...
                     Но тогда зависимости нашего класса все еще остаются неявными. Можно заметить еще один недостаток SL — необходимость использовать implicitly unwrapped optional либо при каждом резолве проверять опционал на nil.
