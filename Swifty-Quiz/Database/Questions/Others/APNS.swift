        ...
        return [
            Question(question: [
                "Для чего используется APNs?"
                     ...
                     helpText: """
                     Отправка пуш уведомлений. APNs расшифровывается как Apple Push Notification service (APNs). Это сервис, который позволяет настроить отправку пуш уведомлений на девайсы клиентов по заданным параметрам (по событиям / по расписанию). Push notifications позволяют разработчикам доносить важную информацию до пользователей, даже если они активно не используют приложение.
                     """
                    ),
            ...
            Question(question: [
                "Может ли пуш уведомление быть получено пользователем, если он в текущий момент не использует приложение?"
                     ...
                     helpText: """
                     Да. Пуш уведомления доставляются на девайс клиента независимо от того, пользуется ли он приложением в текущий момент или нет. Если пользователь при первом запуске разрешил отправку пушей на девайс - он будет получать их независимо от жизненного цикла приложения и его активности.
                     """
                    ),
            ...
            Question(question: [
                "Выберите опции, которые чаще всего используются при получении разрешения на отправку пуш уведомлений?"
                     ...
                     helpText: """
                     alert, sound, badge. Преимущественно используются именно эти три опции, при первичном запросе пользователю на разрешение пуш уведомлений. Они позволяют включить текстовые уведомления, обновление бейджика с количеством непрочитанных уведомлений (на иконке приложения), и добавлять звуки на входящие пуши.
                     """
                    ),
            ...
            Question(question: [
                "Какая опция UNAuthorizationOptions отвечает за разрешение на воспроизведение звука при входящем пуш уведомлении?"
                     ...
                     helpText: """
                     sound. Если при запросе разрешения на пуши вы отправите опцию .sound - вы сможете добавлять стандартный или любой другой кастомный звук для входящего пуш уведомления. Кастомные звуки должны храниться внутри приложения и быть короче 30 секунд.
                     """
                    ),
