...
class GameHelper {
    ...
    func refreshRandomSet(tag: Int) {
        ...
        if tag == 0 {
			SelectedTopic.shared.saveQuestionSet(RandomSetManager.getQuestions(20, .all), topic: "20 случайных вопросов", tag: 0)
        } else if tag == 1 {
            SelectedTopic.shared.saveQuestionSet(RandomSetManager.getQuestions(50, .all), topic: "50 случайных вопросов", tag: 1)
        } else if tag == 2 {
            SelectedTopic.shared.saveQuestionSet(RandomSetManager.getQuestions(100, .all), topic: "100 случайных вопросов", tag: 2)
        } else if tag == 3 {
            SelectedTopic.shared.saveQuestionSet(RandomSetManager.getQuestions(20, .guide), topic: "20 вопросов по Руководству", tag: 3)
        } else if tag == 4 {
            SelectedTopic.shared.saveQuestionSet(RandomSetManager.getQuestions(20, .patterns), topic: "20 вопросов по Паттернам", tag: 4)
        }
    }
